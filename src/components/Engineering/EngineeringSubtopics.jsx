...
const EngineeringSubtopics = () => {
  const { subjectId } = useParams();
  const subject = engineering_sub[0].topics.find(
    (topic) => topic.id === subjectId
  );
  console.log(subject);
    ...
    <div className="container mx-auto px-6 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-8 max-w-5xl mx-auto">
        {subject.subtopics?.map((subtopic) => (
          <Link
            to={`/learn/engineering/${subjectId}/quiz/${subtopic.id}`}
            key={subtopic.id}
...
export default EngineeringSubtopics;
